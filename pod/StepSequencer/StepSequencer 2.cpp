...
float pent[] = {44.f, 67.33f, 88.66f, 128.166f, 155.5f};
...
int main(void)
{
    ...
    //Set up parameters
    decayTimeParam.Init(pod.knob1, .03, 1, decayTimeParam.EXPONENTIAL);
    pitchParam.Init(pod.knob2, 0, 10, pitchParam.EXPONENTIAL);
    ...
    for(int i = 0; i < 15; i++)    //sequencer loop?
    {
        dec[i]    = .5;
        active[i] = false;
        pitch[i]  = 110;
...
void UpdateKnobs()
{
    ...
    else
    {
        ...
        tick.SetFreq(tickFrequency); //speed
        flt.SetFreq(filterFrequency);  //cutoff
