...
daisysp::Comb comb;
const size_t COMB_BUFFER_SIZE = 960;
float comb_buffer[COMB_BUFFER_SIZE];

Parameter ctrls[4];
float ctrl_params[4];
 
size_t process_order = 0;   //setting the variable for PROCESS unsigned int
...
void AudioCallback(AudioHandle::InputBuffer in,
    ...
    float val = in[0][i];  //notice here we dont have any controls pre wired --thye are just iterating through a matrix of array cells
    switch(process_order) {
      ...
      default:
		...
		out[1][i] = out[2][i] = out[3][i] = 0;  //this is interesting 
	}
}

void FlushCombBuffer() {   //try experimenting with the comb buffer size
...
void ProcessControls() {
  ...
  for(size_t c=0; c<4; c++) {
    ctrl_params[c] = ctrls[c].Process(); //4 controls arraay of 4
  }

  if (ctrl_params[0] < 0.1) {  //this makes sure that we dont get stuck at weird MAX and MIN numbers
    ctrl_params[0] = 0.1;
  }
  ...
  if (patch.encoder.Increment() > 0) {  //THIS IS THE KEY TO MAKING THE PROCESS PRESETS SCROLL!!!  everytime there is an increment....
    process_order++;
    FlushCombBuffer();
  }
...
void DisplayLines(const vector<string> &strs) {
  ...
  for (string str : strs) {
    char* cstr = &str[0];
    patch.display.SetCursor(0, line_num*10);     //WHERE WE WRITE
    
    patch.display.WriteString(cstr, Font_7x10, true);   //HERE WE WRITE THE NAMES
    line_num++;
  }
}
