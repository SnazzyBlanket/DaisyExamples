      ...
      do
      {
          ...
          if(file_cnt_ < kMaxFiles - 1)
          {
              if(strstr(fn, ".wav") || strstr(fn, ".WAV"))
              {
                  ...
                  if(f_open(&fil_, file_info_[file_cnt_].name, (FA_OPEN_EXISTING | FA_READ))
                     == FR_OK)
                  {
                    file_sizes[file_cnt_] = f_size(&fil_);
                    char* memoryBuffer = 0;
                    int memorySize= 0;
                    ...
                    if (memoryBuffer)
                    {
                      ...
                      if(f_read(&fil_,(void *)memoryBuffer,size,&bytesread) == FR_OK)
                      {
                        memorySize = size;
